...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    with torch.no_grad():
        ...
        texts = [f"photo of the number {_}" for _ in data_loader.dataset.classes]
        num_text = len(texts)
        text_bs = 256
        text_feats = []
        text_embeds = []
        ...
        for i, sims in enumerate(
            ...
            topk_sim, topk_idx = sims.topk(len(data_loader.dataset.classes), dim=0)
...
def main(args, config):
    ...
    test_dataset = SVHN(
        root="/net/acadia10a/data/zkhan/svhn", split="test", transform=test_transform
    )
    # Manually assign classes to SVHN.
    test_dataset.classes = "zero one two three four five six seven eight nine".split()
